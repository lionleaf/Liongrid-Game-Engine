...
/**
 * The LTexture Library manages all textures in the game.  Textures are pooled and handed out to
 ...
 * a call to loadTexture() or loadAllTextures().  This allows LTexture objects to be dispersed to
 * various game systems and while the texture data itself is streamed in or loaded as necessary.
 */
public class LTextureLibrary extends LBaseObject {
	...
	public LTextureLibrary() {
		super();
		mTextureHash = new LTexture[DEFAULT_SIZE];
		for (int x = 0; x < mTextureHash.length; x++) {
			mTextureHash[x] = new LTexture();
		}
	...
	/**
	 * Creates a LTexture object that is mapped to the passed resource id.  If a texture has already
	 * been allocated for this id, the previously allocated LTexture object is returned.
	 * @param resourceID
	 * @return
	 */
	public LTexture allocateTexture(int resourceID) {
		LTexture texture = getTextureByResource(resourceID);
		if (texture == null) {
	...
	public LTexture loadTexture(Context context, GL10 gl, int resourceID) {
		LTexture texture = allocateTexture(resourceID);
		...
		for (int x = 0; x < mTextureHash.length; x++) {
			if (mTextureHash[x].resource != -1 && mTextureHash[x].loaded) {
				...
				if (error != GL10.GL_NO_ERROR) {
					Log.d("LTexture Delete", "GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + mTextureHash[x].resource);
				}
	...
	protected LTexture loadBitmap(Context context, GL10 gl, LTexture texture) {
		assert gl != null;
		assert context != null;
		assert texture != null;
		if (texture.loaded == false && texture.resource != -1) {
			...
			int error = gl.glGetError();
			if (error != GL10.GL_NO_ERROR) {
				Log.d("LTexture Load 1", "GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
			}
			...
			error = gl.glGetError();
			if (error != GL10.GL_NO_ERROR) {
				Log.d("LTexture Load 2", "GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
			}
			...
			error = gl.glGetError();
			if (error != GL10.GL_NO_ERROR) {
				Log.d("LTexture Load 3", "GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
			}
			...
			error = gl.glGetError();
			if (error != GL10.GL_NO_ERROR) {
				Log.d("LTexture Load 4", "GLError: " + error + " (" + GLU.gluErrorString(error) + "): " + texture.resource);
			}
	 ...
	 * @return An associated LTexture object, or null if there is no associated
	 *  texture in the library.
	 */
	public LTexture getTextureByResource(int resourceID) {
	...
	protected LTexture addTexture(int id, int name, int width, int height) {
