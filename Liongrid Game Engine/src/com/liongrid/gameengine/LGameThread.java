...
public class LGameThread extends Thread { 
	...
	private TouchEventListener mClickListener;
	private long dt;
	...
	public void run() {
		...
		Log.d(LConst.TAG, "Starting LGameThread");
		while(running){
	...
	private synchronized void waitWhilePaused() {
		while(paused){
			try {
				Log.d(LConst.TAG, "Game is paused!");
				wait();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
	...
	private void doTouchEvent(MotionEvent event) {
		...
		mClickListener.onTouch(event);
		//TODO Delegate this to something gamespecific
		//throw new RuntimeException("HAS TO REMOVE doTouchEvent method from engine! Or delegate");
	...
	public void setGameClickListener(TouchEventListener listener){
