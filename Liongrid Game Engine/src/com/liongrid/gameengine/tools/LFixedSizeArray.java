...
/**
 * LFixedSizeArray is an alternative to a standard Java collection like ArrayList.  It is designed
 ...
 */
public class LFixedSizeArray<T> {
    ...
    public LFixedSizeArray(int size) {
        super();
        assert size > 0;
    ...
    public LFixedSizeArray(int size, Comparator<T> comparator) {
        super();
        assert size > 0;
        mContents = (T[])new Object[size];
        mCount = 0;
     ...
     * can leave LFixedSizeArray in an undefined state, so this function is potentially dangerous
    ...
    public void sort(boolean forceResort) {
        if (!mSorted || forceResort) {
           if (mComparator != null) {
               ...
               Log.d("LiongridGameEngine", "No comparator specified for this type, using Arrays.sort().");
	...
	/**
	 * Appends another LFixedSizeArray to the end of this array
	 ...
	 */
	public boolean append(LFixedSizeArray<T> appendee) {
		int appendeeCount = appendee.getCount(); 
		if(mCount + appendee.getCount() > getCapacity()){
			Log.w("LFixedSizeArray", "Index out of bounds during an append:" + mCount+
